...
void function_dx_dt(double t, size_t regime, const gsl_vector *x, double *param, size_t n_param, const gsl_vector *co_variate, gsl_vector *F_dx_dt){
		...
		gsl_vector_set(F_dx_dt,1,-omega * gsl_vector_get(x,0) + param[0] * (1 - pow(gsl_vector_get(x,0), 2)) * gsl_vector_get(x,1));
	}
...
*/void function_dF_dx(double t, size_t regime, double *param, const gsl_vector *co_variate, gsl_matrix *F_dx_dt_dx){
		gsl_matrix_set(F_dx_dt_dx,0,0,0);
		gsl_matrix_set(F_dx_dt_dx,0,1,1);
		gsl_matrix_set(F_dx_dt_dx,1,0,-(param[0] * (2 * param[9+0]) * param[9+1] + omega));
		gsl_matrix_set(F_dx_dt_dx,1,1,param[0] * (1 - pow(param[9+0], 2)));
	}
