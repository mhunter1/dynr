...
C_INFDS getXtildIC3(const int isPar, const int getDxFlag, const int freeIC, struct C_INFDS &InfDS){
	...
	*/
	...
	//dXtildPrev0.print("dXtildPrev0");
	//d2XtildPrev0.print("d2XtildPrev0");
	...
	for (int t = 1; t < T; t++){
		...
		/*
		if (freeIC == 0)
			k1 = dynfunICM(1, XtildPrev, empty_vec, tindex(t), 0, InfDS);
		else
			k1 = dynfunICM(0, XtildPrev, empty_vec, tindex(t), 0, InfDS);
		*/
		k1 = dynfunICM(isPar, XtildPrev, empty_vec, tindex(t), 0, InfDS);
		k21= XtildPrev+dt(t)*k1;
		//[to be checked] case 1 goes for isPar = 1, case 2 goes for isPar = 0
		/*
		if (freeIC == 0)
			k2 = dynfunICM(1, k21, empty_vec, tindex(t)+dt(t), 0, InfDS);
		else
			k2 = dynfunICM(0, k21, empty_vec, tindex(t)+dt(t), 0, InfDS);
		*/
		k2 = dynfunICM(isPar, k21, empty_vec, tindex(t)+dt(t), 0, InfDS);
		Xtild_t = XtildPrev+dt(t)*(k1+k2)/2;
		//Xtild_t.print("Xtild_t");
		xk1.slice(t) = k1;
		xk2.slice(t) = k2;
		...
		if (getDxFlag==1 && t > 1){
			...
			dfdxATXtildprev = dfdxFreeICM(0, XtildPrev, empty_vec, tindex(t), 0, InfDS);
			...
			dfdxATk21 = dfdxFreeICM(0, k21, empty_vec, tindex(t), 0, InfDS);
...
void getScoreInfoY_tobs_opt(struct C_INFDS &InfDS, int stage, int iter, int freeIC, arma::mat &score, arma::mat &infoMat){
	...
	InfDS.dXtildthetafAll(0)(span::all,span(0, 10)).print("InfDS.dXtildthetafAll(0)");
	for (i = 0; i < InfDS.Nsubj; i++){
		...
		for (t = 0; t < T; t++){
			...
			//[HJ note] different original indexY contains indices that satisfies the if; now index Yt contains Ny zeros and ones indicating the corresponding index satisfies the index or not
...
void saem(struct C_INFDS &InfDS, int &gmm, int &stage, int &redFlag, int &convFlag, int &noIncrease, int &stop, double &ssmin, double &ss, arma::mat &sgnTH, arma::vec &mscore, arma::mat &mscore2, arma::mat &minfoMat, arma::mat &Covscore){
	...
	*/
	...
	printf("gain = %lf gainpara= %lf gainparab= %lf\n", gain, gainpara, gainparb);
