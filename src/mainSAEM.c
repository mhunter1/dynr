...
SEXP main_SAEM(SEXP model_list, SEXP data_list, SEXP weight_flag_in, SEXP debug_flag_in, SEXP optimization_flag_in, SEXP hessian_flag_in, SEXP verbose_flag_in)
{
	...
	/*isFreeIC: */
	SEXP isFreeIC_sexp = PROTECT(getListElement(model_list, "freeIC"));
	bool freeIC_bool=*LOGICAL(isFreeIC_sexp);
	...
	*/
	...
	if (total_time_all_subj_int  > 0){		
		...
		for(int s = 0; s < Nsubj; s++){
			...
			for(i = 0; i < allT[s];i++){
				if (i!= 0)
						printf(";");
				printf(" %d", tobs[total_time_all_subj_int]);
				total_time_all_subj_int ++;
	...
	if (total_time_all_subj_int  > 0){
		...
		for(int s = 0; s < Nsubj; s++){
			...
			for(i = 0; i < allT[s];i++){
				if (i!= 0)
						printf(";");
				printf(" %.3lf", timeDiscrete[total_time_all_subj_int]);
				total_time_all_subj_int ++;
	...
	SET_VECTOR_ELT(res, 0, ScalarInteger(output.convFlag));   
	SET_VECTOR_ELT(res, 1, ScalarInteger(output.nIterStage1));    
	SET_VECTOR_ELT(res, 2, ScalarInteger(output.nIterStage2));    
	SET_VECTOR_ELT(res, 3, ScalarReal(output.ss));       
	SET_VECTOR_ELT(res, 4, ScalarReal(output.avebAccept));       
	SET_VECTOR_ELT(res, 5, iytild_sexp);   
