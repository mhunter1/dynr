...
\item The Rtools Setup Wizard will appear in a window. Follow the Setup Wizard until it reaches the ``Select Destination Location'' page. 
\item On the page says ``Select Destination Location'' at the top, by default, it will suggest to install R in ``C:\textbackslash Rtools'' on your computer. You may also install R-tools in  {\em any other directory where there are {\bf no spaces} in the words describing the directory}. Follow through the Setup Wizard after that. 
