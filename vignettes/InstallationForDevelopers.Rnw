...
\item The following instructions are based on rtools40.
\item Click RTools 4.0, then click ``rtools40-x86\_64.exe`` in the new webpage. You may be asked if you want to save or run a file ``rrtools40-x86\_64.exe``. Choose ``Save'' and save the file on the Desktop. Then double-click on the icon for the file to run it.
\item (If you are asked what language to install it in during installation - choose English.)
\item The next page says ``Select Destination Location`` at the top. By default, it will suggest to install R in ``C:\textbackslash rtools40'' on your computer. You may also install R-tools in  {\em any other directory where there are {\bf no spaces} in the words describing the directory}. Click ``Next'' at the bottom of the R tools Setup wizard window.
...
\\
 C:\textbackslash R\textbackslash R-4.1.1\textbackslash bin\textbackslash x64\\
\texttt{C:\textbackslash rtools40\textbackslash usr\textbackslash bin\\
 C:\textbackslash rtools\textbackslash mingw64\textbackslash bin\\
...
\label{sec:GitHub}
\begin{enumerate}
\item Create a GitHub account if you don't already have one (\url{https://github.com/}). Email Michael D. Hunter (\href{mailto:midohu@gmail.com}{midohu@gmail.com}) for permission to dynr's GitHub repository.
