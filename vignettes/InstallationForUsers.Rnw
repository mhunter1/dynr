...
\subsection{\Large Wrapping up the Installation Procedure }
\label{wrapup}
\begin{enumerate}
...
\item If the above failed, then something went wrong with either Section \ref{sec:gsl} and/or Section \ref{sec:libgsl}. Please go back and try repeating or checking those sections.
