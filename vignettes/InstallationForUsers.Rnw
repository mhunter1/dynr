...
\begin{enumerate}
 ...
 \end{enumerate}
 \item Use Windows system to set LIB\_GSL.
 \begin{enumerate}
 ...
 directory containing the local323 files downloaded in Section \ref{sec:gsl} or the directory where your GSL libraries are installed. Example \texttt{"C:\textbackslash Program Files\textbackslash R\textbackslash local323"} but please note that {\em the quotation marks should be included in the variable value}.
 \end{enumerate}
\end{enumerate}
...
\begin{enumerate}
 ...
 \item If the file did not run correctly, follow {\em one} of the following:
