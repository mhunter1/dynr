...
\begin{enumerate}
 ...
 directory containing the local323 files downloaded in Section \ref{sec:gsl}. Example {C:/R/local323} but please note that the direction of these slashes is important. This slash / will work but {\em not} this one \textbackslash.
\end{enumerate}
...
\item Open RGui, Rstudio or whatever editor you use to run R. Please type the following code to check whether the gsl commands can be found correctly:\\ \texttt{shell("echo \%LIB\_GSL\%")}\\ If this returns something like ``C:/R/local323'', then everything worked fine.
 \item If that command returns something like ``\%LIB\_GSL\%'', then something is wrong with the GSL path.  Return to Section \ref{sec:libgsl}.
...
\item If the above failed, then something went wrong with either Section \ref{sec:macpre}. Please go back and try repeating or checking that section.
