...
setMethod("writeCcode", "dynrDynamicsFormula",
	function(object, covariates){
		...
		if (object@isContinuousTime){
			#TODO test if both if(nregime > 1){}else{} breaks (for dx_dt and dF_dx) can be eliminated by using the switch statement with only one regime
			#function_dx_dt
			...
			if (nregime > 1){ # more than one regime (regime-switching)
				ret <- paste(ret, "switch (regime) {", sep="\n\t")
				for (r in 1:nregime){
					ret <- paste(ret, paste0("case ", r-1, ":"), sep="\n\t")
